...
class CheapTriager:
    ...
    def triage(self, proposal: CandidateProposal, profile: dict) -> dict:
        scope = scope_decision(profile, proposal.asset, "GET")
        excluded = proposal.finding_category in profile["excluded_finding_categories"]
        linked = len(set(proposal.source_record_ids + proposal.observation_ids))
        evidence_completeness = min(1.0, linked * 0.35 +
            (0.2 if proposal.observed_fact else 0) + (0.15 if proposal.missing_evidence else 0))
        ...
        triage_score = round(
            proposal.estimated_impact * 0.30 + proposal.confidence * 0.25 +
            proposal.novelty * 0.15 + evidence_completeness * 0.20 +
            reproducibility * 0.10, 6)
        expected_value = round(
            proposal.estimated_impact * proposal.confidence * proposal.novelty / estimated_cost, 6)
